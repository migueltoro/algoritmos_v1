...
public record MonedasVertex(Integer index,Integer valorRestante) 
	implements  VirtualVertex<MonedasVertex, MonedasEdge, Integer> {
	...
	public static MonedasVertex first(Integer valorInicial) {
		return new MonedasVertex(0,valorInicial);
	}
	
	public static MonedasVertex last() {
		return new MonedasVertex(DatosMonedas.n,0);
	}
	...
	public MonedasVertex copy() {
		return MonedasVertex.of(this.index(), this.valorRestante());
	}
	...
	public MonedasVertex neighbor(Integer a) {
		MonedasVertex r;
		if(this.valorRestante() == 0) r = MonedasVertex.last();
		else r = MonedasVertex.of(this.index()+1,this.valorRestante()-a*DatosMonedas.valor(this.index()));
		return r;
	}
